...
import sys
import numpy as np
from spextractor_common import ParaRowsCols, ParaWords, pr, SpexHush
...
class SpexRandDrop(SpexHush):
    ...
    def gen_words_indexes_as_lists_para(self):
        ...
        shufflek = np.random.choice(len(col), int(len(col) * self.pref_drop), replace=False)
        col = [col[i] for i in shufflek]
        shufflek = np.random.choice(len(lig), int(len(lig) * self.suff_drop), replace=False)
    ...
    def proba_words_special(self, words, asdict=True):
        pad = int(self.model.input.shape[1])  # On déduit de la taille de la couche d'entrée le pad nécéssaire
        suffs_batch = set()
...
if __name__ == "__main__":
    ...
    spex = SpexRandDrop(arg_model, arg_lrows, arg_lcols, 1.0, 1.0, arg_testfile, arg_testtargetsfile, context_a)
    spex.ready()
